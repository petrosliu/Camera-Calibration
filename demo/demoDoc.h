...
public:
	//保存一个图像的副本，因为CDemoView类会修改图像的内容
	CxImage *duplicate_image;				
	...
	int pixelCount;            //保存种子填充区域的象素个数
	long circle_x;             //保存中心的x坐标
	long circle_y;             //保存中心的y坐标
	...
	/////////////////////////////////////////////
	CxImage *m_pInverseImg;
public:
	void InverseImage(int nType);
	CxImage *GetInverseImg();


public:
	bool FloodFill(CPoint iPt,CxImage *img,bool *bMark,int,int,int);
	bool UnFloodFill(CxImage *img,bool *bMark);
	bool IsSameColor(COLORREF c, COLORREF o, int tolerance_r, int tolerance_g, int tolerance_b);
};


inline CxImage* CDemoDoc::GetInverseImg()
{
	return m_pInverseImg;
}
/////////////////////////////////////////////////////////////////////////////
