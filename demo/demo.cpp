...
BEGIN_MESSAGE_MAP(CDemoApp, CWinApp)
	...
	ON_COMMAND(ID_CALIBRATION_BATCH_IMAGERESIZE, OnCalibrationBatchImageresize)
	ON_COMMAND(ID_IMGPOINT_LOADCTRLPNT, OnImgpointLoadctrlpnt)
	ON_COMMAND(ID_IMPORT_LIGHT_POINT, OnImportLightPoint)
	ON_COMMAND(ID_CALIBRATION_BATCH_IMAGECUT, OnCalibrationBatchImagecut)
	//}}AFX_MSG_MAP
...
CDemoApp::CDemoApp()
{
	...
	memset(m_CtrlPts,0,sizeof(POINT3D)*36*10);

	m_bIsLoadCtrlPts=FALSE;
	nSurfSum=0;
	nPtSum=0;
...
//批量文件处理，更改图像大小和格式。
void CDemoApp::OnCalibrationBatchImageresize() 
{

	CDlgCalibBatchImgReSize dlgCalibBtResize;
	if(dlgCalibBtResize.DoModal()==IDOK)
	{
	}
...
//读取控制点文件
void CDemoApp::OnImgpointLoadctrlpnt() 
{
	char strFilter[]="(*.txt)|*.txt||";
	CFileDialog fDlg(TRUE,"txt","*.txt",OFN_OVERWRITEPROMPT,strFilter);
	fDlg.m_ofn.lpstrTitle="Ground Control File:";

	if(fDlg.DoModal()==IDOK)
	{
		m_bIsLoadCtrlPts=TRUE;

		CString fileName=fDlg.GetPathName();		
		FILE *fp=fopen(fileName,"r");
		fscanf(fp,"%d %d",&nSurfSum, &nPtSum);
		float    gX,gY,gZ;
		int      row=0,col=0;
		for(int j=0;j<nSurfSum;j++)
		{
			for(int i=0;i<nPtSum;i++)
			{
				fscanf(fp,"%d %d %f %f %f",&row,&col,&gX,&gY,&gZ);

				m_CtrlPts[j][row*6+col].x=gX;
				m_CtrlPts[j][row*6+col].y=gY;
				m_CtrlPts[j][row*6+col].z=gZ;
			}
		}
		fclose(fp);
		CString strMsg;
		strMsg.Format("成功导入控制点(物体坐标系) %d*%d个",nSurfSum,nPtSum);
...
void CDemoApp::OnImportLightPoint() 
{
	char strFilter[]="(*.txt)|*.txt||";
	CFileDialog fDlg(TRUE,"txt","*.txt",OFN_OVERWRITEPROMPT,strFilter);
	fDlg.m_ofn.lpstrTitle="Ground Control File:";
	...
	if(fDlg.DoModal()==IDOK)
	{
		...
		for(int i=0;i<nPtSum;i++)
		{
			fscanf(fp,"%d %d %f %f",&row,&col,&gX,&gY);
				
			m_CtrlPts_light[row*6+col].x=gX;
			m_CtrlPts_light[row*6+col].y=gY;
		}		
		fclose(fp);

		AfxMessageBox("成功导入控制点(光源坐标系)");
	}		
}
...
CDemoApp * GetApp()
{
	return (CDemoApp *) AfxGetApp();
}


#include "DlgBatchImgCut.h"
void CDemoApp::OnCalibrationBatchImagecut() 
{
	// TODO: Add your command handler code here
	CDlgBatchImgCut dlgBatchImgCut;
	if(dlgBatchImgCut.DoModal()==IDOK)
	{
	}
}
