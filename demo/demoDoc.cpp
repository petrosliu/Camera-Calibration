...
CDemoDoc::CDemoDoc()
{
	...
	m_pInverseImg = NULL;
	seed = new CPoint[100000];
	ptStack = new int[10000000];
}
...
BOOL CDemoDoc::OnOpenDocument(LPCTSTR lpszPathName) 
{
	...
	duplicate_image=new CxImage(filename, type);
	Stopwatch(1);
		...
		if (AfxMessageBox(s,MB_OKCANCEL)==IDOK){
			...
			for(int i=1;i<image->GetNumFrames();i++){
				...
				if (NewDoc)	
				{
					...
					NewDoc->duplicate_image=newImage;
...
//--------------------------------------
bool CDemoDoc::IsSameColor(COLORREF c,COLORREF o,
						 int tolerance_r,int tolerance_g,int tolerance_b)
{
	int or=GetRValue(o);
	int og=GetGValue(o);
	int ob=GetBValue(o);
	...
	bool Same=TRUE;
	if( fabs(b-ob)< tolerance_b && fabs(r-or)<tolerance_r && fabs(g-og)<tolerance_g )
 		Same=TRUE;
	else
		Same=FALSE;
...
//			bMark: 标识象素是否被扫描	
//--------------------------------------
bool CDemoDoc::UnFloodFill(CxImage *img,bool *bMark)
{

	int nWid=img->GetWidth();
	int x,y;

	for(long i = 0;i < ptPointer; )
	{
		x = ptStack[i ++];
		y = ptStack[i ++];
		bMark[y * nWid + x] = false;
	}
...
//			bMark: 标识象素是否被扫描	
//--------------------------------------
bool CDemoDoc::FloodFill(CPoint iPt, CxImage *img,bool *bMark,
						 int tolerance_r,int tolerance_g,int tolerance_b)
{
	...
	//取得种子的颜色
	COLORREF oc;
	oc=img->RGBQUADtoRGB(img->GetPixelColor(iPt.x,iPt.y ));
	...
	//左右填充情况
	bool bFillLeft = true;
	bool bFillRight = true;
	
	//如果堆栈不为空，就一直循环
	while(stackPoint != 0)
	{
		ASSERT(stackPoint <= 100000);

		//取出种子
		currPos.x = seed[stackPoint].x;
		currPos.y = seed[stackPoint].y;
		stackPoint --;

		bFillLeft = true;
		bFillRight = true;

		bMark[currPos.y * nWid + currPos.x] = true;
		ptStack[ptPointer++] = currPos.x;
		ptStack[ptPointer++] = currPos.y;
		pixelCount ++;
		circle_x += currPos.x;
		circle_y += currPos.y;
		...
		while(bFillLeft)
		{
			...
			{
				tempColor=img->RGBQUADtoRGB(img->GetPixelColor(currPos.x,currPos.y));
				//扫描到符合要求的点
				if(IsSameColor(oc,tempColor,tolerance_r,tolerance_g,tolerance_b)
					&& !bMark[currPos.y * nWid + currPos.x])
				{
					bMark[currPos.y * nWid + currPos.x] = true;
					ptStack[ptPointer++] = currPos.x;
					ptStack[ptPointer++] = currPos.y;
					...
					circle_y += currPos.y;
					//记录扫描区域的范围
					if(currPos.x < min_x) min_x = currPos.x;
					if(currPos.y < min_y) min_y = currPos.y;
					if(currPos.x > max_x) max_x = currPos.x;
					if(currPos.y > max_y) max_y = currPos.y;
				}
				else
				{
					xl = currPos.x + 1;
		...
		if(currPos.y >= 0)
		{
			...
			while(currPos.x <= xr)
			{
				pFlag = false;
				tempColor=img->RGBQUADtoRGB(img->GetPixelColor(currPos.x,currPos.y));
				while(IsSameColor(oc,tempColor,tolerance_r,tolerance_g,tolerance_b) 
					&& !bMark[currPos.y * nWid + currPos.x] 
					&& currPos.x <= xr)
				{
					if(!pFlag) pFlag = true;
				...
				if(pFlag)
				{
					if(currPos.x == xr && IsSameColor(oc,tempColor,tolerance_r,tolerance_g,tolerance_b) && !bMark[currPos.y * nWid + currPos.x])
					{
						stackPoint ++;
						seed[stackPoint].x = currPos.x;
						seed[stackPoint].y = currPos.y;
					}
					else
					{
						stackPoint ++;
						seed[stackPoint].x = currPos.x - 1;
						seed[stackPoint].y = currPos.y;
					}
					pFlag = false;
				}//if
				int xEnter = currPos.x;
				while(currPos.x < xr &&  
					(!IsSameColor(oc,tempColor,tolerance_r,tolerance_g,tolerance_b) 
					|| bMark[currPos.y * nWid + currPos.x]))
				{
					currPos.x ++;
					tempColor=img->RGBQUADtoRGB(img->GetPixelColor(currPos.x,currPos.y));
				}//while
				if( currPos.x == xEnter)  currPos.x ++;
			}//while
		}//if
...
void CDemoDoc::InverseImage(int nType)
{
	if(image == NULL)
		AfxMessageBox("请打开一幅图片!");

	if(m_pInverseImg != NULL)
	{
		delete m_pInverseImg;
		m_pInverseImg = NULL;
	}

	m_pInverseImg = new CxImage;
	m_pInverseImg->Copy(*image);
	RGBQUAD tempRGB;

	int nHeight = m_pInverseImg->GetHeight();
	int nWidth = m_pInverseImg->GetWidth();
	int row,col;
	...
	case 1:
		for(row = 0; row < nHeight; row++)
		{
			...
			{
				tempRGB = image->GetPixelColor(col,row);
				m_pInverseImg->SetPixelColor(col,nHeight-1-row,tempRGB);
			}
		}
		break;
	case 2:
